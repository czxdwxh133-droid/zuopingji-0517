...
在浏览资讯文章时，用户经常面临“标题感兴趣但无法判断内容价值”的问题，逐篇阅读原文不仅耗时，也降低了信息筛选效率。针对这一场景，设计并实现资讯速览agent，通过理解网页结构、自主调用工具和大模型分析能力，实现从文章获取到摘要生成的自动化流程。

用户只需输入 URL，Agent 即可自主判断任务流程：

- 识别资讯列表页或单篇文章，并选择对应处理策略；
- 自动提取文章内容并调用大模型生成摘要；
- 根据执行结果动态调整后续任务；
- 实时反馈处理状态和最终摘要结果。
